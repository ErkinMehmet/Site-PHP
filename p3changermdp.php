<?php include('p3header.php'); ?>
<a class="active" href="p3.php">Log In</a>
<a href="p3info.php">Info</a>
</div>

<!-- POUR COMM
<div class="content">
hello this is contenido
<div contentEditable="true" class="commentbox">ddfaf
</div>
</div>
-->
<div class="content">
<div class="ligne">

<div class="gauche">


</div>

<div class="centre">
    <h2>Log In to your UniGymnacio</h2>
<?php include('conexion.php'); 
//info pour la personne
$id=$_GET['id'];
$quer="select * from usuarios where id=".$id;
$existe=$bdd->query($quer);
while ($existe2=$existe->fetch()) {
    $email=$existe2['email'];
    $nom=$existe2['nom'];
}


?>
<form action="p3changercont.php" method="post">
<label>Username: <?php echo htmlspecialchars($nom); ?></label>
<label>Email: <?php echo htmlspecialchars($email); ?></label>
<label for="mdp">Password:</label>
<input type="password" name="mdp"/>
<label for="mdp2">Confirmed Password:</label>
<input type="password" name="mdp2"/>

<input type="hidden" name="id" value="<?php echo $id;?>">

<input type="submit"/>
</form>

<a href="p3inscription.php">Not signed up yet?</a><br/>
<a href="p3oublie.php">Forgot password?</a>






</div>


<div class="droite">

</div>

</div>
</div>


<?php include('p3footer.php'); ?>